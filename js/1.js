var dat = [ { label: 'My franchise should always do whatever it takes to win championships', b: 379 },
            { label: 'My franchise should focus on winning championships, but also be ok with short periods of moderate success', b: 585 },
            { label: 'My franchise should mostly be ok with success without championship contention, but not for too long', b: 152 },
            { label: "My franchise should always be ok with moderate success, even if it's not a championship contender", b: 42 }
            ];
        ...
        // 5) Add the previous elements to the graph, and add user-specified color.
            ...
            var path = svg.selectAll('path');
                    ...
                    .on('mouseover', function(d) {
                        tip.show(d);
                        d3.select(this).style("fill", "#24D7B7");
                    })
                    ...
                    // Animation on load
                    .transition().delay(function(d,i) { return i * 1000; }).duration(3000)
